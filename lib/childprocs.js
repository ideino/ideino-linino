...
exports.connect = function(io, sessionStore) {
	...
	function onspawn(options, callback) {
        ...
        var autorun_path = path.normalize("/opt/ideino-autorun");
	...
	function onspawnautorun(options, callback){
		...
		var path = require('path');
		var p2 = path.normalize("/opt/ideino-autorun");//path.normalize(__dirname+path.sep+".."+path.sep+"autorun")
		p2 = path.join(p2,'autorun.sh');
		
		var opts = [];
		var prj_dir = options.paths.shift();
		//#2.1 params - autorun shell script absolute path
		opts.push(p2);
		//#2.2 params - src project directory absolute path
		opts.push(prj_dir);
		//#2.3 params - relative path of the .js executable file
		opts.push(path.join(path.sep,utils.getPath(options.paths)));
		//console.log(opts);		
